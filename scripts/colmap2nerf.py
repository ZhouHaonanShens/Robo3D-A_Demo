...
def parse_args():
    ...
    parser.add_argument("--colmap_dense", default="colmap_dense",
                        help="input path to the colmap dense files (set automatically if run_colmap is used)")
    parser.add_argument("--colmap_db", default="colmap.db", help="colmap database filename")

    parser.add_argument("--run_colmap_dense", action="store_true", help="run colmap for dense reconstruction")
...
def run_colmap(args):
    ...
    dense_flag = args.run_colmap_dense
    ...
    if dense_flag:
        dense_workspace = db_noext + "_dense_workspace"
        max_image_dimension = 640
        do_system(
            f"colmap image_undistorter --image_path {images} --input_path {sparse}/0 --output_path {dense_workspace} \
            --output_type COLMAP --max_image_size {max_image_dimension}")
        dense_stereo = db_noext + "_dense_stereo"
        do_system(f"colmap patch_match_stereo --workspace_path {dense_workspace}")
...
if __name__ == "__main__":
    ...
    if args.hold <= 0:
        ...
        write_json('transforms_ready.json', frames)
