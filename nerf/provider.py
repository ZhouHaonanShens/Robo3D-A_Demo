...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            if opt.depth_supervise or opt.depth_supervise_E2:
                self.depths = []
                for f in frames:
                    depth_path = os.path.join(self.root_path, f['file_path'])
                    if self.mode == 'blender' and 'npy' not in os.path.basename(depth_path):
            ...
            # if opt.hz_train:
            #     self.hashtable = transform["hashtable"]
    ...
    def collate(self, index):
        ...
        if self.depths is not None:
            depths = self.depths[index].to(self.device)
            # depths = depths.reshape()
            if self.training:
                C = 1  # depths.shape[-1]
                depths = torch.gather(depths.view(B, -1, C), 1, torch.stack(C * [rays['inds']], -1))
