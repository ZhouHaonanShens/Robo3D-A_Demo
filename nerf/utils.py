...
def read_new_frames(frames, opt, scale, offset, H, W, device):
    poses = []
    images = []
    depths = []
    root_path = opt.path
    ...
    depth_path = os.path.join(root_path, f['file_path'])
    if 'npy' not in os.path.basename(depth_path):
        depth_path = depth_path[:-4] + '_depth.npy'  # so silly...

        if not os.path.exists(depth_path):
            print('File dose not exist')
            return False, False, False

        depth_np = np.load(depth_path) * (0.92 / 0.55)
    ...
    if depths is not None:
        depths = torch.from_numpy(np.stack(depths, axis=0))  # [N, H, W, 1]

    if opt.preload:
        poses = poses.to(device)
        if images is not None:
            # TODO: linear use pow, but pow for half is only available for torch >= 1.10 ?
            if opt.fp16 and opt.color_space != 'linear':
                dtype = torch.half
            else:
                dtype = torch.float
            images = images.to(dtype).to(device)
        if depths is not None:
            depths = depths.to(torch.half).to(device)

    return poses, images, depths
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        # pred_weights = outputs['weights']

        # MSE loss
        loss = self.criterion(pred_rgb, gt_rgb).mean(-1)  # [B, N, 3] --> [B, N]
        if self.opt.depth_supervise:
            z_vals = outputs['z_vals']
            weights = outputs['weights']
            depths = data['depths']
            dists = outputs['dists']
            # avg_cdo = 4.63676202881754
            # depth_scale = (4.0/avg_cdo)*self.opt.scale
            # depth_scale = 0.89 / 0.54
            depth_scale = 0.62
            max_depth = 3 * depth_scale
            depths = depths * depth_scale
            # max_depth = 15 * depth_scale
            err = torch.ones_like(z_vals)
            # err[depths[:, None] > max_depth] = 500
            # z_vals[z_vals.isnan()] = 3*max_depth
            # z_vals = torch.nan_to_num(z_vals, nan=1.2*max_depth, posinf=1.2*max_depth)
            np_depth = depths.detach().cpu().numpy()

            idxs_min = 0.1 < np_depth  # < max_depth
            idxs_max = np_depth < max_depth
            idxs = np.array(idxs_min * idxs_max, dtype=np.bool)
            idxs = np.reshape(idxs, -1)
            # loss_depth = -torch.log(weights + 1e-8) * torch.exp(-(z_vals - depths[:, None]) ** 2 / (2 * err)) * dists
            loss_depth = torch.zeros((1, weights.shape[0], weights.shape[1]), device=weights.device).double()
            loss_depth[:, idxs, :] = -torch.log(weights[idxs, ...] + 1e-12) * \
                                  torch.exp(-(z_vals[idxs, ...] - depths[:, idxs, :]) ** 2 /
                                            (2 * err[idxs, ...])) * dists[idxs, ...]
            loss_depth = torch.sum(loss_depth, dim=-1)
            loss_depth = loss_depth.reshape(loss.shape)

            # loss_depth = torch.nan_to_num(loss_depth, nan=1.0, posinf=1.0)
            if not (True in loss_depth.isnan()):
                # d_rate = 0.003*torch.exp(-(torch.tensor(self.global_step/(self.opt.iters-self.global_step + 1))))
                loss += 0.005 * loss_depth
                # loss += d_rate * loss_depth
            # loss = -torch.log(weights) * torch.exp(-(z_vals - depths[:, None]) ** 2 / (2 * err)) * dists

        # Here We can edit the Loss Function!
        depth_loss = torch.tensor(0, dtype=loss.dtype)
        depth_loss_scale = torch.tensor(0.5, dtype=loss.dtype)
        if self.opt.depth_supervise_E2:
            depth_scale = 0.62
            depths = data['depths'][:, :, 0].to(dtype=loss.dtype) * depth_scale
            ...
            depth_loss = self.criterion(pred_depth[0, idxs], depths[0, idxs]).mean().float()
        ...
        loss = loss.mean()
        if self.opt.depth_supervise_E2:
            loss += depth_loss.mean() * depth_loss_scale
    ...
    @torch.no_grad()
    def get_entropy_step_np(self, rays_o, rays_d, H, W):
        outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=None, perturb=True, force_all_rays=True,
                                    **vars(self.opt))
        _weights = outputs['weights']
        _weights = _weights.cpu().numpy().reshape(H, W, -1)
        w_fine_entropy = np.sum(
            -_weights * np.log(_weights + 1e-6), axis=-1
        )
        return w_fine_entropy

    @torch.no_grad()
    def get_entropy_step(self, rays_o, rays_d, H, W):
        outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=None, perturb=True, force_all_rays=True,
                                    **vars(self.opt))
        _weights = outputs['weights']
        _weights = _weights.to(self.device)
        w_fine_entropy = torch.sum(
            -_weights * torch.log(_weights + 1e-6), dim=-1
        )
        return w_fine_entropy

    @torch.no_grad()
    def entropy_step(self, data):
        poses = data['poses']
        rays_o = data['rays_o']
        rays_d = data['rays_d']
        # print(poses)
        w_fine_entropy = self.get_entropy_step_np(rays_o, rays_d, data['H'], data['W'])
        # print(w_fine_entropy)
    ...
    def hz_train(self, train_set, valid_set, max_epochs, init_epochs=50, update_interval=2, zone_num=6):
        train_loader = train_set.dataloader()
        valid_loader = valid_set.dataloader()
        rootpath = train_set.root_path

        data_path = os.path.join(rootpath, 'transforms_train_all.json')
        json_file = 'transforms_train_all.json' if os.path.isfile(data_path) else 'transforms_train.json'
        data_path = os.path.join(rootpath, json_file)
        # fp = open(data_path, 'r')
        frames = None
        hashtable = None
        with open(data_path, 'r') as j:
            ...
            hashtable = j_data['hashtable']
            del j_data
        # fp.close()

        selecting_terminated = False
        if self.use_tensorboardX and self.local_rank == 0:
            self.writer = tensorboardX.SummaryWriter(os.path.join(self.workspace, "run", self.name))
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if epoch > init_epochs \
                    and (epoch - init_epochs + update_interval) % update_interval == 0 \
                    and (not selecting_terminated):
                # update the train_set
                plus_phi = 5
                if self.last_selected_pose is None:
                    self.last_selected_pose = {
                        "theta": 54,
                        "phi": (-plus_phi)
                    }
                p_dposes = []
                for scan_idx in range(1, zone_num + 1):
                    scan_phi = self.last_selected_pose["phi"] + plus_phi * scan_idx
                    if scan_phi > 360 - plus_phi:
                        if self.last_selected_pose["theta"] != 66:
                            self.last_selected_pose = {
                                "theta": 66,
                                "phi": 0
                            }
                            ...
                            selecting_terminated = True
                        break
                    new_pose = {
                        "theta": self.last_selected_pose['theta'],
                        "phi": scan_phi
                    }
                    p_dposes.append(new_pose)

                entropies = []
                p_idxs = []
                for p_dpose in p_dposes:
                    p_hashidx = str(p_dpose["theta"] * 1000 + p_dpose["phi"])
                    p_idx = hashtable[p_hashidx]
                    p_idxs.append(p_idx)
                    frame = frames[p_idx]
                    p_pose = np.array(frame['transform_matrix'], dtype=np.float32)
                    p_pose = nerf_matrix_to_ngp(p_pose, scale=train_set.scale, offset=train_set.offset)
                    p_pose = torch.tensor(np.array([p_pose]), device=train_set.device)
                    s = np.sqrt(train_set.H * train_set.W / train_set.num_rays)  # only in training, assert num_rays > 0
                    rH, rW = int(train_set.H / s), int(train_set.W / s)
                    rays = get_rays(p_pose, train_set.intrinsics / s, rH, rW, -1)
                    w_fine_entropy = self.get_entropy_step_np(rays['rays_o'], rays['rays_d'], rH, rW)
                    entropies.append(np.mean(w_fine_entropy))

                if entropies:
                    entropies = np.array(entropies)
                    # decays = [0.8, 0.9, 0.95, 0.98]
                    # if decays:
                    #     entropies[:min(len(decays), len(entropies))] = \
                    #         entropies[:min(len(decays), len(entropies))] * decays[:min(len(decays), len(entropies))]
                    max_idx = np.argmax(entropies)
                    frame = frames[p_idxs[max_idx]]

                    # update the train set with the selected frame
                    n_poses, n_images, n_depths = read_new_frames(frame, train_set.opt,
                                                                  train_set.scale, train_set.offset,
                                                                  train_set.H, train_set.W,
                                                                  train_set.device)

                    if n_poses is not False:
                        train_set.poses = torch.cat((train_set.poses, n_poses), dim=0)
                        train_set.images = torch.cat((train_set.images, n_images), dim=0)
                        if train_set.depths is not None:
                            train_set.depths = torch.cat((train_set.depths, n_depths), dim=0)
                        train_loader = train_set.dataloader()

                        if not line_change:
                            self.last_selected_pose = p_dposes[max_idx]
                        else:
                            line_change = False
                        print(p_dposes[max_idx])
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        if write_video:
            ...
            imageio.mimwrite(os.path.join(save_path, f'{name}_rgb.mp4'), all_preds, fps=15, quality=8,
                             macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_depth.mp4'), all_preds_depth, fps=15, quality=8,
                             macro_block_size=1)
